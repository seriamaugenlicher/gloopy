...
{
	ss.begin_section(SaveState::fourcc("TIME"));

	//Save states are only taken between frames, where no timer is mid-slice
	for (int i = 0; i < NUM_TIMERS; i++)
	{
		Timer* timer = &state.timers[i];
		assert(!timer->in_slice);

		ss.write(timer->timestamp);
		ss.write(timer->next_event_id);

		uint32_t count = (uint32_t)timer->events.size();
		ss.write(count);
		for (auto& ev : timer->events)
		{
			ss.write(ev.exec_time);
			ss.write(ev.param);
			ss.write(ev.id);
			ss.write_string(state.funcs[ev.func_index].name);
		}
	}
}
...
{
	ss.expect_section(SaveState::fourcc("TIME"));

	for (int i = 0; i < NUM_TIMERS; i++)
	{
		Timer* timer = &state.timers[i];

		ss.read(timer->timestamp);
		ss.read(timer->next_event_id);
		timer->slice_length = 0;
		timer->in_slice = false;
		timer->set_cycles_left(0);
		...
		{
			Event ev;
			ss.read(ev.exec_time);
			ss.read(ev.param);
			ss.read(ev.id);
			std::string func_name = ss.read_string();

			//Match the event back to a function registered in this session. An event whose
			//owner isn't present (e.g. sound disabled now but enabled at save time) is dropped.
			int func_index = -1;
			for (size_t k = 0; k < state.funcs.size(); k++)
			{
				if (state.funcs[k].name == func_name)
				{
					func_index = (int)k;
					break;
				}
			...
			if (func_index < 0)
			{
				Log::warn("[Timing] dropping save state event for unregistered func %s", func_name.c_str());
				continue;
			}

			ev.func = state.funcs[func_index].func;
			ev.func_index = func_index;
			timer->events.push_back(ev);
		}
