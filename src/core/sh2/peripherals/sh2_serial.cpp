...
struct Port
{
	...
	void sched_tx_ev()
	{
		//The port index (not a pointer) is used as the event param so save states can serialize it
		Timing::UnitCycle sched_cycles = Timing::convert_cpu(cycles_per_bit);
		tx_ev = Timing::add_event(tx_ev_func, sched_cycles, (uint64_t)id, Timing::CPU_TIMER);
	}
};
...
{
	assert(!cycles_late);
	Port* port = &state.ports[param % PORT_COUNT];
...
{
	ss.begin_section(SaveState::fourcc("SCI "));
	for (auto& port : state.ports)
	{
		ss.write(port.tx_ev.value);
		ss.write(port.bit_factor);
		ss.write(port.cycles_per_bit);
		ss.write(port.mode);
		ss.write(port.ctrl);
		ss.write(port.status);
		ss.write(port.tx_bits_left);
		ss.write(port.tx_shift_reg);
		ss.write(port.tx_buffer);
		ss.write(port.tx_prepared_data);
	}
}

void load_state(SaveState::Snapshot& ss)
{
	ss.expect_section(SaveState::fourcc("SCI "));
	for (auto& port : state.ports)
	{
		//The tx event itself is restored by Timing::load_state; the handle stays
		//consistent because event ids are preserved
		ss.read(port.tx_ev.value);
		ss.read(port.bit_factor);
		ss.read(port.cycles_per_bit);
		ss.read(port.mode);
		ss.read(port.ctrl);
		ss.read(port.status);
		ss.read(port.tx_bits_left);
		ss.read(port.tx_shift_reg);
		ss.read(port.tx_buffer);
		ss.read(port.tx_prepared_data);
	}
}
